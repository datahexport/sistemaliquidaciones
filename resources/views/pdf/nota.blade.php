...
<body>
	...
	<div class="cuerpo" style="display: none;">
	...
	<div class="cuerpo" >
		...
		<div style="display: flex; justify-content: space-between; font-family: Arial, sans-serif; font-size: 12px; margin-bottom: 10px; align-items: center;">
			...
			<div style="text-align: right;">
			  ...
			  <span style="margin-left: 5px;">{{number_format(($totaldentrodenorma+$totalfueradenorma+$totalembalada-$materiales-$certificaciones-$multiresiduos+$resultadocomercial),2)}} USD</span>
			</div>
		  </div>
		  @php
			  $total_liquidacion=$totaldentrodenorma+$totalfueradenorma+$totalembalada-$materiales-$certificaciones-$multiresiduos+$resultadocomercial;
		  @endphp
		...
		<table id="balance" style="width:100%; border-collapse: collapse; margin-top: 20px;">
            ...
            $totalfacturas_iva=0;
				...
				@foreach($facturas as $item)
					...
					@php
						$totalfacturas+=$item->cantidad;
            $totalfacturas2+=$item->monto_neto;
             $totalfacturas_iva+=$item->iva;
					@endphp
				@endforeach

				
				@if (floatval($total_liquidacion)>floatval($totalfacturas))
					<tr>
						...
						<td style="padding-bottom: 4px; margin-top: 10px; text-align: right; padding-right: 10;white-space: nowrap;">
							{{ number_format($total_liquidacion-$totalfacturas, 2, ',', '.') }} USD
						</td>
					</tr>
				@else
					<tr>
				...
				@if($item->tc>0)
					...
					<tr>
						...
						<td style="padding-bottom: 4px; margin-top: 10px; text-align: right; padding-right: 10;">
							{{ number_format($informe_edit->diferencia_tipodecambio*0.19, 0, ',', '.') }}
						</td>
						<td style="padding-bottom: 4px; margin-top: 10px; text-align: right; padding-right: 10;">
						</td>
						<td style="padding-bottom: 4px; margin-top: 10px; text-align: right; padding-right: 10;">
		...
		<table id="balance" style="width:100%; border-collapse: collapse; margin-top: 20px;">
          ...
          <td style="text-align: left; padding-left: 25px;" colspan="">
         		...
         		{{ number_format(($total_liquidacion-$totalfacturas)*floatval($temporada->tc)+$totalfacturas2+$informe_edit->diferencia_tipodecambio, 0, ',', '.') }}
					...
					<td style="text-align: right; padding-right: 10;">
						{{ number_format($total_liquidacion, 2, ',', '.') }} USD
					</td>
				</tr>
			</tbody>
		</table>
              ...
              @endif
				...
				<tr style="background-color: #ffffff; font-weight: bold;">
					...
					<td style="text-align: right; padding-left: 25px;">
						{{number_format(($total_liquidacion-$totalfacturas)*floatval($temporada->tc)+$informe_edit->diferencia_tipodecambio, 0, ',', '.')}}
					</td>
				...
				<tr style="background-color: #ffffff; font-weight: bold;">
					...
					<td style="text-align: right; padding-left: 25px;">
						{{number_format((($total_liquidacion-$totalfacturas)*floatval($temporada->tc)+$informe_edit->diferencia_tipodecambio)*0.19, 0, ',', '.')}}
					</td>
				...
				<tr style="background-color: #ffffff; font-weight: bold;">
					...
					<td style="text-align: right; padding-left: 25px;">
						{{number_format((($total_liquidacion-$totalfacturas)*floatval($temporada->tc)+$informe_edit->diferencia_tipodecambio)*1.19, 0, ',', '.')}}
					</td>
			...
			<tbody>
             ...
             @if (floatval($total_liquidacion)>floatval($totalfacturas))
              ...
              @endif
				...
				<tr style="background-color: #ffffff; font-weight: bold;">
					<td style="text-align: left; padding-left: 25px;">
						Mayor Valor Resultado Liquidación de Cerezas Exportación Temporada {{$temporada->name}}
					</td>
