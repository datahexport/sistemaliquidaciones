        ...
        {  
            foreach($rows as $row){
                Balancemasatres::create([
                    ...
                    'C_Mercado' => $row[43],
                    'Nave' => $row[44],
                    'Fecha_Salida' => Carbon::instance(Date::excelToDateTimeObject(intval($row[45]))),
                    'Exportador' => $row[46],
                    'Mercado' => $row[47],
                    'Despacho' => $row[48],
                    'Folio_Sag' => $row[49],
                    'Fecha_despacho' => Carbon::instance(Date::excelToDateTimeObject(intval($row[50]))),
                    'Recibidor_comprador' => $row[51],
                    'Numero_Inspeccion' => $row[53],
                    'Fecha_Inspeccion' => Carbon::instance(Date::excelToDateTimeObject(intval($row[54]))),
                    'Estado_Inspeccion' => $row[55],
                    'Guia_sii' => $row[56],
                    'Contenedor' => $row[57],
                    'Peso_Neto' => $row[58],
                    'Peso_Bruto' => $row[59],
                    'Fecha_Digitacion' => Carbon::instance(Date::excelToDateTimeObject(intval($row[60]))),
                    'N_Reserva' => $row[61],
                    'Fecha_Reserva' => Carbon::instance(Date::excelToDateTimeObject(intval($row[62]))),
                    'Planta' => $row[63],
