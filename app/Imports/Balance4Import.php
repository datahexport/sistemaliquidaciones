            ...
            foreach($rows as $row){
                if (is_numeric($row[0]) && is_numeric($row[1])) {
                    Balancemasacuatro::create([
                        ...
                        'Sales_date' => Carbon::instance(Date::excelToDateTimeObject($row[0])),
                        'Arrival_Date' => Carbon::instance(Date::excelToDateTimeObject($row[1])),
                        'N_Pallet' => $row[2],
